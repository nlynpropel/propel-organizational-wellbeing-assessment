/*
# Fix: generate_recommendations reads section scores from DB, not snapshot

## Problem
The result_snapshot does not contain a `section_scores` key — it only has
`overall_score`, `behavioral_readiness`, `score_band`, and `computed_at`.
The RPC's LATERAL joins on `jsonb_array_elements(v_section_scores)` return
nothing because the array is empty, so dimension scores are never matched.
All recommendations default to score 100, causing:
  - Strengths (need >= 75) to never trigger
  - Priority opportunities (need < 75) to never trigger
  - Quick wins and high-impact moves (depend on priorities) to never trigger

## Fix
Build a section score map from the `assessment_section_scores` table joined
with `assessment_sections` (using the section title slug as dimension key),
instead of reading from the snapshot. This is the same approach used in the
`20260721180300` version of the RPC.

Behavioral readiness is still read from the snapshot (it IS present there).

## Security
No schema changes. Only the `generate_recommendations` function is recreated.
*/
...
BEGIN
  ...
  -- Build section score map from DB: { dimension_key -> normalized_score }
  ...
  LEFT JOIN public.assessment_section_scores ss
    ON ss.section_id = s.id AND ss.assessment_instance_id = v_instance_id
  ...
  SELECT array_agg(DISTINCT o.option_value) INTO v_selected_priority_tags
  ...
  FOR v_rec IN
    SELECT r.*,
      ...
      COALESCE((v_behavioral_readiness->r.driver_key)::numeric, 0) as drv_score
    ...
    ORDER BY GREATEST(
      ...
      COALESCE((v_behavioral_readiness->r.driver_key)::numeric, 0)
  ...
  FOR v_rec IN
    SELECT r.*,
      ...
      COALESCE((v_behavioral_readiness->r.driver_key)::numeric, 100) as drv_score
    FROM public.recommendations r
    WHERE r.framework_id = v_framework_id
      AND r.is_active = true
      AND r.recommendation_type IN ('quick_win', 'high_impact_move')
      ...
      AND GREATEST(
        ...
        COALESCE((v_behavioral_readiness->r.driver_key)::numeric, 100)
      ) < 75
    ORDER BY GREATEST(
      COALESCE((v_sec_score_map->r.dimension_key)::numeric, 100),
      COALESCE((v_behavioral_readiness->r.driver_key)::numeric, 100)
