-- 1. Add report configuration columns to assessment_templates
ALTER TABLE assessment_templates
  ADD COLUMN IF NOT EXISTS report_type text NOT NULL DEFAULT 'scored',
  ADD COLUMN IF NOT EXISTS maturity_enabled boolean NOT NULL DEFAULT true,
  ADD COLUMN IF NOT EXISTS section_scores_enabled boolean NOT NULL DEFAULT true,
  ADD COLUMN IF NOT EXISTS behavioral_driver_scores_enabled boolean NOT NULL DEFAULT true,
  ADD COLUMN IF NOT EXISTS respondent_result_mode text NOT NULL DEFAULT 'broker_only';

-- Set the Propel Well-being Opportunity Index to full scored report
UPDATE assessment_templates
SET report_type = 'scored',
    scoring_enabled = true,
    maturity_enabled = true,
    section_scores_enabled = true,
    behavioral_driver_scores_enabled = true,
    recommendations_enabled = true,
    respondent_result_mode = 'broker_only'
WHERE name ILIKE '%well-being opportunity index%' OR name ILIKE '%propel wellbeing%';

-- 2. Update resolve_reusable_link to include report config fields
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'template_respondent_result_mode', v_template.respondent_result_mode,
...
-- 3. Update submit_reusable_assessment to call finalize_assessment_submission for scoring
...
BEGIN
  ...
  -- This ensures reusable-link submissions get identical scoring as conventional sends
